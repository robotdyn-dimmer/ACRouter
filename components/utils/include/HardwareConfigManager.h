...
namespace HardwareConfigKeys {
    ...
    constexpr const char* I2C_SDA_GPIO      = "i2c_sda";
    constexpr const char* I2C_SCL_GPIO      = "i2c_scl";
    constexpr const char* I2C_FREQ          = "i2c_freq";      // stored as uint16 in kHz
    constexpr const char* I2C_ENABLED       = "i2c_en";
    constexpr const char* I2C1_SDA_GPIO     = "i2c1_sda";
    constexpr const char* I2C1_SCL_GPIO     = "i2c1_scl";
    constexpr const char* I2C1_FREQ         = "i2c1_freq";     // stored as uint16 in kHz
    constexpr const char* I2C1_ENABLED      = "i2c1_en";
    constexpr const char* RBAMP_BUS         = "rbamp_bus";     // rbAmp fleet bus (0/1)
    constexpr const char* RBAMP_DRDY        = "rbamp_drdy";    // rbAmp DRDY GPIO (0xFF=off)
    constexpr const char* ESPNOW_CHANNEL    = "espnow_ch";     // ESP-NOW WiFi channel (1-13)
    constexpr const char* ESPNOW_ENABLED    = "espnow_en";     // ESP-NOW transport enable

    // Internal ADC enable flag (can be disabled when I2C provides all measurements)
    constexpr const char* ADC_ENABLED       = "adc_en";
...
struct HardwareConfig {
    // ADC Channels (4 max, legacy config; ADC pipeline removed in v2.0)
    ...
    // I2C Bus Configuration
    uint8_t i2c_sda_gpio;       ///< I2C SDA pin (default: 21)
    uint8_t i2c_scl_gpio;       ///< I2C SCL pin (default: 22)
    uint32_t i2c_freq_hz;       ///< I2C clock frequency (default: 100000)
    bool i2c_enabled;           ///< I2C bus enabled (default: false)

    // I2C Bus 1 (optional second bus). Lets a module be moved off bus 0 to
    // avoid shared-bus read contention (e.g. DimmerLink telemetry corrupted by
    // rbAmp's fast poll — put DimmerLink on bus 1). Per-module bus assignment:
    // DimmerLink devices carry i2c_bus; rbAmp fleet uses rbamp_i2c_bus below.
    uint8_t i2c1_sda_gpio;      ///< I2C bus-1 SDA pin (default: 19)
    uint8_t i2c1_scl_gpio;      ///< I2C bus-1 SCL pin (default: 23)
    uint32_t i2c1_freq_hz;      ///< I2C bus-1 clock frequency (default: 100000)
    bool i2c1_enabled;          ///< I2C bus-1 enabled (default: false)
    uint8_t rbamp_i2c_bus;      ///< Bus the rbAmp fleet polls on (0 or 1, default 0)
    int8_t rbamp_drdy_gpio;     ///< rbAmp DRDY GPIO for interrupt-driven poll (-1 = timer poll)

    // ESP-NOW transport (wireless module bus) — groundwork; nodes come later
    uint8_t espnow_channel;     ///< ESP-NOW WiFi channel (1-13, must match nodes; default 1)
    bool espnow_enabled;        ///< ESP-NOW transport enabled (default false)

    // Internal ADC
    bool adc_enabled;           ///< Internal ADC enabled (default: true, can disable for I2C-only)
...
public:
    ...
    /**
     * @brief Configure the optional second I2C bus (bus 1) and persist it.
     * @param sda SDA GPIO   @param scl SCL GPIO
     * @param freq_hz clock frequency in Hz   @param enabled enable bus 1
     * @return true if saved successfully
     */
    bool setI2CBus1(uint8_t sda, uint8_t scl, uint32_t freq_hz, bool enabled);

    /**
     * @brief Assign the rbAmp fleet to an I2C bus (0 or 1) and persist it.
     * @param bus bus number (0 or 1)
     * @return true if saved (false on invalid bus)
     */
    bool setRbAmpBus(uint8_t bus);

    /**
     * @brief Set the rbAmp DRDY GPIO for interrupt-driven polling and persist it.
     * @param gpio DRDY input GPIO, or < 0 to disable (fixed-cadence timer poll)
     * @return true if saved successfully
     */
    bool setRbAmpDrdy(int gpio);
