...
bool ConfigManager::setControlGain(float gain) {
    // Floor matches RouterController's runtime clamp (MIN_CONTROL_GAIN=10). Previously
    // this stored down to 1 while the control loop re-clamped to 10 — a value < 10 was
    // saved verbatim but silently applied as 10. Unify so stored == applied.
...
bool ConfigManager::setGridCurrentLimit(float amps) {
    if (amps < 0.0f) amps = 0.0f;
    if (amps > 100.0f) amps = 100.0f;
    m_config.grid_current_limit = amps;
    return saveFloat(ConfigKeys::GRID_CURRENT_LIMIT, amps);
}
...
bool ConfigManager::loadAll() {
    ...
    success &= loadFloat(ConfigKeys::GRID_CURRENT_LIMIT, m_config.grid_current_limit, ConfigDefaults::GRID_CURRENT_LIMIT);
...
bool ConfigManager::saveAll() {
    ...
    success &= saveFloat(ConfigKeys::GRID_CURRENT_LIMIT, m_config.grid_current_limit);
