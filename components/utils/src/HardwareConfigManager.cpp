...
void HardwareConfig::setDefaults() {
    ...
    // Zero-cross detector — v2.0: dimming is offloaded to DimmerLink smart
    // modules (onboard zero-cross), so the ESP32 GPIO zero-cross is OFF by
    // default. Enable only for a legacy GPIO/TRIAC (rbdimmer) dimmer path.
    ...
    // LED indicators. The ESP32 defaults (PIN_LED_STATUS=17, PIN_LED_LOAD=5) collide on
    // the ESP32-C2: GPIO5 is the C2 I2C SDA and GPIO12-17 are the in-package flash. Use
    // C2-safe free GPIOs there (10/7 — outside I2C 5/6, strapping 8/9, flash 12-17, UART
    // 19/20). Adjust via /api/hardware/config if the board wires LEDs elsewhere.
    ...
    // I2C bus — v2.0: the smart-module (rbAmp) sensing bus is the default path.
    // Pin defaults are target-aware: the ESP32-C2 (ESP8684) has a very different,
    // much smaller GPIO map (21/22 don't exist; 12-17 are the in-package flash;
    // 8/9 strapping; 19/20 UART0), so default to GPIO5/6 there.
#if CONFIG_IDF_TARGET_ESP32C2
    i2c_sda_gpio = 5;
    i2c_scl_gpio = 6;
#else
    ...
    // I2C bus 1 — optional second bus, off by default. When enabled, a module
    // can be assigned here to avoid shared-bus contention with rbAmp.
    i2c1_sda_gpio = 19;
    i2c1_scl_gpio = 23;
    i2c1_freq_hz = 100000;
    i2c1_enabled = false;
    rbamp_i2c_bus = 0;
    rbamp_drdy_gpio = -1;   // DRDY interrupt-driven poll off by default (timer poll)

    // ESP-NOW transport — off by default; channel must match wireless nodes.
    espnow_channel = 1;
    espnow_enabled = false;

    // Internal ADC — v2.0 clean cut: sensing is offloaded to rbAmp (I2C) /
    // ESP-NOW smart modules, so the local ADC is OFF by default. Enable only for
    // legacy direct-ADC hardware (an existing NVS config keeps its saved value).
    adc_enabled = false;
}
...
bool HardwareConfigManager::begin() {
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ...
        // Safe mode active - sensing sources start in a reduced state
...
bool HardwareConfigManager::setI2CBus1(uint8_t sda, uint8_t scl, uint32_t freq_hz, bool enabled) {
    m_config.i2c1_sda_gpio = sda;
    m_config.i2c1_scl_gpio = scl;
    m_config.i2c1_freq_hz  = freq_hz;
    m_config.i2c1_enabled  = enabled;

    bool ok = saveU8(HardwareConfigKeys::I2C1_SDA_GPIO, sda);
    ok &= saveU8(HardwareConfigKeys::I2C1_SCL_GPIO, scl);
    ok &= saveU16(HardwareConfigKeys::I2C1_FREQ, (uint16_t)(freq_hz / 1000));
    ok &= saveBool(HardwareConfigKeys::I2C1_ENABLED, enabled);
    return ok;
}

bool HardwareConfigManager::setRbAmpBus(uint8_t bus) {
    if (bus >= 2) return false;
    m_config.rbamp_i2c_bus = bus;
    return saveU8(HardwareConfigKeys::RBAMP_BUS, bus);
}

bool HardwareConfigManager::setRbAmpDrdy(int gpio) {
    int8_t v = (gpio < 0) ? -1 : (int8_t)gpio;
    m_config.rbamp_drdy_gpio = v;
    return saveU8(HardwareConfigKeys::RBAMP_DRDY, (uint8_t)v);
}
...
bool HardwareConfigManager::saveAll() {
    ...
    // Save I2C bus config
    success &= saveU8(HardwareConfigKeys::I2C_SDA_GPIO, m_config.i2c_sda_gpio);
    success &= saveU8(HardwareConfigKeys::I2C_SCL_GPIO, m_config.i2c_scl_gpio);
    success &= saveU16(HardwareConfigKeys::I2C_FREQ, (uint16_t)(m_config.i2c_freq_hz / 1000));
    success &= saveBool(HardwareConfigKeys::I2C_ENABLED, m_config.i2c_enabled);
    success &= saveU8(HardwareConfigKeys::I2C1_SDA_GPIO, m_config.i2c1_sda_gpio);
    success &= saveU8(HardwareConfigKeys::I2C1_SCL_GPIO, m_config.i2c1_scl_gpio);
    success &= saveU16(HardwareConfigKeys::I2C1_FREQ, (uint16_t)(m_config.i2c1_freq_hz / 1000));
    success &= saveBool(HardwareConfigKeys::I2C1_ENABLED, m_config.i2c1_enabled);
    success &= saveU8(HardwareConfigKeys::RBAMP_BUS, m_config.rbamp_i2c_bus);
    success &= saveU8(HardwareConfigKeys::RBAMP_DRDY, (uint8_t)m_config.rbamp_drdy_gpio);
    success &= saveU8(HardwareConfigKeys::ESPNOW_CHANNEL, m_config.espnow_channel);
    success &= saveBool(HardwareConfigKeys::ESPNOW_ENABLED, m_config.espnow_enabled);

    // Save ADC enable flag
    success &= saveBool(HardwareConfigKeys::ADC_ENABLED, m_config.adc_enabled);
...
bool HardwareConfigManager::loadAll() {
    ...
    loadBool(HardwareConfigKeys::ZEROCROSS_EN, m_config.zerocross_enabled, false);
    ...
    // Load I2C bus config (defaults: disabled, SDA=21, SCL=22, 100kHz)
    loadU8(HardwareConfigKeys::I2C_SDA_GPIO, m_config.i2c_sda_gpio, 21);
    loadU8(HardwareConfigKeys::I2C_SCL_GPIO, m_config.i2c_scl_gpio, 22);
    {
        uint16_t freq_khz = 100;
        loadU16(HardwareConfigKeys::I2C_FREQ, freq_khz, 100);
        m_config.i2c_freq_hz = (uint32_t)freq_khz * 1000;
    }
    loadBool(HardwareConfigKeys::I2C_ENABLED, m_config.i2c_enabled, true);

    // Load I2C bus-1 config (defaults: disabled, SDA=19, SCL=23, 100kHz)
    loadU8(HardwareConfigKeys::I2C1_SDA_GPIO, m_config.i2c1_sda_gpio, 19);
    loadU8(HardwareConfigKeys::I2C1_SCL_GPIO, m_config.i2c1_scl_gpio, 23);
    {
        uint16_t freq1_khz = 100;
        loadU16(HardwareConfigKeys::I2C1_FREQ, freq1_khz, 100);
        m_config.i2c1_freq_hz = (uint32_t)freq1_khz * 1000;
    }
    loadBool(HardwareConfigKeys::I2C1_ENABLED, m_config.i2c1_enabled, false);
    loadU8(HardwareConfigKeys::RBAMP_BUS, m_config.rbamp_i2c_bus, 0);
    {
        uint8_t drdy = 0xFF;   // 0xFF sentinel = disabled (-1)
        loadU8(HardwareConfigKeys::RBAMP_DRDY, drdy, 0xFF);
        m_config.rbamp_drdy_gpio = (int8_t)drdy;
    }
    loadU8(HardwareConfigKeys::ESPNOW_CHANNEL, m_config.espnow_channel, 1);
    loadBool(HardwareConfigKeys::ESPNOW_ENABLED, m_config.espnow_enabled, false);

    // Load ADC enable flag (v2.0 clean cut: default OFF — smart-module sensing)
    loadBool(HardwareConfigKeys::ADC_ENABLED, m_config.adc_enabled, false);
...
void HardwareConfigManager::printConfig() const {
    ...
    // I2C Bus
    ESP_LOGI(TAG, "I2C Bus:");
    ESP_LOGI(TAG, "  SDA: GPIO%d, SCL: GPIO%d, Freq: %lu Hz  %s",
             m_config.i2c_sda_gpio, m_config.i2c_scl_gpio,
             (unsigned long)m_config.i2c_freq_hz,
             m_config.i2c_enabled ? "ENABLED" : "DISABLED");

    // Internal ADC
    ESP_LOGI(TAG, "Internal ADC: %s", m_config.adc_enabled ? "ENABLED" : "DISABLED");
