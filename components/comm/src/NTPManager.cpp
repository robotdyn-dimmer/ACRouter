...
static const char* NVS_NAMESPACE = "ntp";

// Helper function to check if STA is connected (replaces WiFi.status() == WL_CONNECTED)
...
bool NTPManager::begin(const char* ntp_server, const char* timezone,
    ...
    // Try to load from NVS (overrides defaults)
    loadConfig();
    ...
    // Configure SNTP with loaded/default values
    esp_sntp_setoperatingmode(SNTP_OPMODE_POLL);
    esp_sntp_setservername(0, _ntp_server.c_str());

    // Set sync interval to 1 hour (in milliseconds)
    ...
    ESP_LOGI(TAG, "  NTP Server: %s", _ntp_server.c_str());
...
bool NTPManager::loadConfig() {
    nvs_handle_t nvs;
    esp_err_t err = nvs_open(NVS_NAMESPACE, NVS_READONLY, &nvs);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "NVS namespace 'ntp' not found, using defaults");
        return false;
    }
    ...
    // Load ntp_server
    len = sizeof(buffer);
    if (nvs_get_str(nvs, "ntp_server", buffer, &len) == ESP_OK) {
        _ntp_server = String(buffer);
    }

    // Load timezone
    len = sizeof(buffer);
    if (nvs_get_str(nvs, "timezone", buffer, &len) == ESP_OK) {
        _timezone = String(buffer);
    }

    // Load offsets
    int32_t val;
    if (nvs_get_i32(nvs, "gmt_offset", &val) == ESP_OK) {
        _gmt_offset_sec = val;
    }
    if (nvs_get_i32(nvs, "dst_offset", &val) == ESP_OK) {
        _daylight_offset_sec = val;
    }

    nvs_close(nvs);
    ESP_LOGI(TAG, "NTP configuration loaded from NVS");
    ESP_LOGI(TAG, "  Server: %s", _ntp_server.c_str());
    ESP_LOGI(TAG, "  Timezone: %s", _timezone.c_str());
    return true;
}
...
bool NTPManager::saveConfig() {
    ...
    nvs_set_str(nvs, "ntp_server", _ntp_server.c_str());
    nvs_set_str(nvs, "timezone", _timezone.c_str());
    nvs_set_i32(nvs, "gmt_offset", _gmt_offset_sec);
    nvs_set_i32(nvs, "dst_offset", _daylight_offset_sec);

    err = nvs_commit(nvs);
    nvs_close(nvs);

    if (err == ESP_OK) {
        ESP_LOGI(TAG, "NTP configuration saved to NVS");
        return true;
    } else {
