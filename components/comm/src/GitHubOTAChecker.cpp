...
bool GitHubOTAChecker::fetchLatestRelease() {
    ...
    // ESP32-C2: TLS/HTTPClient stack is trimmed (arduino ssl_client is disabled
    // on this SoC and mbedTLS is too heavy for 272 KB SRAM). The GitHub update
    // check (HTTPS) is therefore unavailable here; local OTA upload still works.
    ESP_LOGW(TAG, "GitHub update check unavailable on esp32c2 (no HTTPS stack)");
    return false;
#else
    HTTPClient http;
