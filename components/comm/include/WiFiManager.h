 ...
 *
 * Uses ESP-IDF native WiFi API for better control over task priorities
 * and to avoid conflicts with timing-critical components like AC dimmer.
 *
...
struct WiFiConfig {
    ...
    WiFiConfig() :
        // Default off: drop the AP once STA is connected. AP+STA holds a second netif
        // + DHCP server + beacon buffers, and on the RAM-tight C2 (~13 KB free) that
        // headroom is the difference between surviving and OOMing under HTTP load. The
        // AP auto-restarts if STA drops (fallback config access preserved).
        ap_always_on(false),
...
class WiFiManager {
    // Allow event handler to access private members
    friend void wifi_event_handler(void* arg, esp_event_base_t event_base,
                                   int32_t event_id, void* event_data);
public:
    /**
